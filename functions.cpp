...
double approximation_exp_func(double *coef,double x) //линеаризация показательной функции 
{
    return log(coef[0]) * (coef[1] * x + coef[2]);
}



void least_squares(int n,double *x,double *coef,double (*f)(double *coef, double x),double *error_coef) //мнк 
{
    ...
    double delta_b = delta_a * sqrt(Sx + medium_x * medium_x);//погрешность для b
}